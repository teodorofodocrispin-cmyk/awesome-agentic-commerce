...
- [Intelica](https://intelica.onrender.com) — Competitive intelligence API for autonomous AI agents. Analyzes any URL or company description, returns structured JSON with market positioning, competitors, pain points, and executable Market Score (`threat_level`, `moat_strength`, `agent_recommendation`). 10 context modes including `crypto_protocol`, `regulatory_compliance`, `venture_screening`, `risk_assessment`, `sales_enablement`. Standard $0.05 USDC / Elite $1.00 USDC on Base + Solana. MCP at `/mcp`, A2A at `/message/send`. ([x402 manifest](https://intelica.onrender.com/.well-known/x402.json), [OpenAPI](https://intelica.onrender.com/openapi.json))
